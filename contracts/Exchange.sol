...
contract Exchange is ERC20 {
    ...
    // Echange is inherting ERC20, because our exchange itself is an ERC-20 contract
    ...
    //getReserve returns the balancee of 'token' held by contract
    ...
    function addLiquidity(
        ...
        if (tokenReserveBalance == 0) {
            ...
            // lpTokensToMint = etherReserveBalance = msg.value
        ...
        require(
            ...
            "Insufficent amount of tokens provided"
        );

        //Transfer the token from the uset to the exchange

        token.transferFrom(msg.sender, address(this), minTokenAmountRequired);

        // Calculate the amount od LP tokens to be minted
        lpTokensToMint =
    ...
    // removeLiquidity allows users to remove liquidity from Exchange
    ...
    function removeLiquidity(
        ...
        // Burn the LP tokens from the user, and transfer the ETH and tokens to user
    ...
    // getOutputAmountFromSwap calculate the amount of to ve received based on xy = (x + dx)(y - dy)
    ...
    // ethTokenSwap allows users to swap ETH for tokens
